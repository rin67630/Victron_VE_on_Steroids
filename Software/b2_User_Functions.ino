#if defined (D7_IS_DROK)          // this code option is not yet tested, waiting for hardware to arrive...
// Serial Handling Drok PSU converters
void HandleNewData()
{
  float m;
  m = payload.BatV;
  payload.BatV = psu.readVoltageMilliVolts() / 1000;
  dBatV = payload.BatV - m;      // Used for internal resistance evaluation in Stats

  m = payload.BatI;
  payload.BatI = psu.readCurrentMilliAmps() / 1000;
  dBatI = payload.BatI - m;      // Used for internal resistance evaluation in Stats
  setpoints.BatV = psu.readSetMilliVolts() / 1000;
  setpoints.BatI = psu.readSetMilliAmps()  / 1000;
  psu.getOutputEnabled();
}
#endif
